...
For this controller/panel pair, the normal supported topology is simpler than
the repository originally documented:
...
That means controller-distributed panel power is an intended configuration,
not an unsupported workaround. Matrix Studio can therefore target a **single
external power cable in normal deployed use**, with data arriving over Wi-Fi.
...
That choice remains valid after correcting the board identity:

- native ESP-IDF gives direct control over Wi-Fi, task affinity and memory
- `esp-hub75` provides the ESP32-S3 DMA path and the FM6124 driver mode needed
  by the physical panel
- render and network work remain separated so the sustained 24 FPS Wi-Fi/DMA
  test can expose any real contention rather than hiding it behind framework
  behaviour
...
3. ❓ whether this panel batch uses RGB, RBG, or another colour ordering at the
   logical display level
